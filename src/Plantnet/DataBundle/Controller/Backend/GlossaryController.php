    ...
    public function glossary_syn_updateAction($id)
    {
        ...
        if('POST'===$request->getMethod()){
            ...
            if($form->isValid()){
                ...
                $kernel=$this->get('kernel');
                $command=$this->container->getParameter('php_bin').' '.$kernel->getRootDir().'/console publish:glossary syn '.$id.' '.$user->getDbName().' '.$user->getEmail().' &> /dev/null &'; //' &> /dev/null &';
                $process=new \Symfony\Component\Process\Process($command);
                $process->start();
        ...
        if('POST'===$request->getMethod()){
            ...
            if($form->isValid()){
                if(unlink($csv)){
                    ...
                    $dm->getConfiguration()->setDefaultDB($this->getDataBase($user,$dm));
                    $kernel=$this->get('kernel');
                    $command=$this->container->getParameter('php_bin').' '.$kernel->getRootDir().'/console publish:glossary unsyn '.$id.' '.$user->getDbName().' '.$user->getEmail().' &> /dev/null &'; //' &> /dev/null &';
                    $process=new \Symfony\Component\Process\Process($command);
                    $process->start();
                    $this->get('session')->getFlashBag()->add('msg_success','Glossary updated');
                }
